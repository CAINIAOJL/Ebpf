...
//define min raw function for c, not c++ function
#define min(x, y)                       \
    ({                                  \
        typeof(x) _min1 = (x);          \
        typeof(y) _min2 = (y);          \
...
static int SSL_exit(struct pt_regs *ctx, int rw) {
    ...
    data->rw = rw;
    data->buf_filled = 0;
    data->is_handleshake = false;
    u32 buf_copy_size = min((size_t)len, (size_t)MAX_BUF_SIZE);
    ...
    if(ret == 0) {
        //读取成功，标志buf_filled为1-true
        data->buf_filled = 1;
    } else {
        buf_copy_size = 0;
    }
    //将数据写入perf_SSL_events
...
int BPF_URETPROBE(probe_SSL_do_handshake_exit) {
    ...
    data->rw = 2;
    data->buf_filled = 0;
    data->is_handleshake = true;
    bpf_get_current_comm(&data->comm, sizeof(data->comm));
    bpf_map_delete_elem(&start_ns, &tid);

    bpf_perf_event_output(ctx, &perf_SSL_events, BPF_F_CURRENT_CPU, data, EVENT_SIZE(0));
    return 0;
}
