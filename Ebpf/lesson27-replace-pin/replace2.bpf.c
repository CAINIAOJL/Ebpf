...
int BPF_PROG(find_possible_addrs, struct pt_regs *regs, long ret) {
    ...
    for(unsigned int i = 0; i < loop_size; i++) {
        ...
        for(unsigned int j = 0; j < local_buff_size; j++) {
            //预测点，只要第一个字符相同，预测可能成功（是我们想要的，也有可能不是）
...
SEC("fexit/__x64_sys_read")
int BPF_PROG(check_possible_address, struct pt_regs *regs, long ret) {
    size_t pid_tgid = bpf_get_current_pid_tgid();
    long unsigned int *pbuff_addr = bpf_map_lookup_elem(&map_buff_addrs, &pid_tgid);
    ...
    for(unsigned int i = 0; i < MAX_POSSIBLE_ADDRS; i++) {
        ...
        //bpf_strncmp(pFind->text, TEXT_LEN_MAX, (const char*)name_cmp)
...
SEC("fexit/__x64_sys_read")
int BPF_PROG(overwrite_address, struct pt_regs *regs, long ret) {
    size_t pid_tgid = bpf_get_current_pid_tgid();
    long unsigned int *pbuff_addr = bpf_map_lookup_elem(&map_buff_addrs, &pid_tgid);
    ...
    for(unsigned int i = 0; i < MAX_POSSIBLE_ADDRS; i++) {
        ...
        bpf_probe_read_user(&data, TEXT_LEN_MAX, (char*)name_addr);
