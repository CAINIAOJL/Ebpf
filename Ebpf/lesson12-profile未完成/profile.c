...
int main(int argc, char** argv) {
    const char* online_cpus_file = "/sys/devices/system/cpu/oonline";
    int freq = 1, pid = -1, cpu;
    struct profile_bpf* bpf; //骨架
    ...
    if(err) {
        fprintf(stderr, "Failed to parse online cpu mask file %s\n", online_cpus_file);
        goto cleanup;
    }
    ...
    ring_buffer = ring_buffer__new(bpf_map__fd(skel->maps.events), event_handler, NULL, NULL);
    ...
    for(cpu = 0; cpu < num_cpus; cpu++) {
        ...
        if(pefd < 0) {
            fprintf(stderr, "Failed to open perf event for cpu %d\n", cpu);
            err = -1;
            goto cleanup;
        }
    ...
    while(ring_buffer__poll(ring_buffer, -1) >= 0) {
