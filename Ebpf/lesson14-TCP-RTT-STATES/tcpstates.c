...
//volatitle 只是告诉编译器，这个变量可能会被其他线程修改，但是不保证线程安全，需要原子操作或者加锁来保证安全。
static volatile sig_atomic_t exiting = 0; //如果一个变量修饰了volatile，那么它就是一个易变的变量，即使它位于寄存器中，也能保证它的修改能及时地被其他线程看到。

static bool emit_timestamp = false;
static short target_family = 0;
static char* target_sports = NULL; //source-prots 集合
...
static error_t parse_arg(int key, char* arg, struct argp_state *state) {
    ...
    case 'T':
        ...
        break;
    case '4':
        target_family = AF_INET; //ipv4
        break;
    case '6':
        target_family = AF_INET6; //ipv6
        break;
    case 'W':
        wide_output = true;
        break;
...
static int libbpf_print_fn(enum libbpf_print_level level, const char* format, va_list args) {
    // libbpf_print_fn(LIBBPF_INFO, "xxxxx", a, v, b);
    if(level == LIBBPF_DEBUG && !versbose) {
        return 0;
    }
