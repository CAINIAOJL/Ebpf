...
static struct prog_env {
    ...
    unsigned int interval;  //输出间隔
    unsigned int iterations;  //输出次数
    bool verbose;
    bool timestamp;
    bool kprobes;
    char *funcname;
...
const char *argp_program_version = "funclate 0.1";
...
static error_t parse_arg(int key, char *arg, struct argp_state *state) {
    ...
    switch (key) {
        ...
        case 'm':
            if(env->unist != NSEC) {
                warn("Conflicting options: -m and -u\n");
                argp_usage(state);
            }
            env->unist = MSEC;
            break;
...
static int attach_uprobe(struct funclate *skel) {
    ...
    binary = strdup(env.funcname); /*strdup( )在内部调用了malloc( )为变量分配内存，不需要使用返回的字符串时，需要用free( )释放相应的内存空间，否则会造成内存泄漏。*/
    if(!binary) {
        warn("Failed to allocate memory for binary name\n");
        return -1;
    }
