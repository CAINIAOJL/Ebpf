...
int tracepoint_syscalls__sys_enter_open(struct trace_event_raw_sys_enter *ctx) {
    ...
    if(rewrite) {
        bpf_probe_write_user((char *)ctx->args[1], "hijacked", sizeof("hijacked"));
    }
...
static __always_inline int trace_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    ag = bpf_map_lookup_elem(&start, &pid);
    if(!ag) {
        return 0;
    }
    ...
    event->ret = ret;
    event->flags = ag->flags;
    bpf_get_current_comm(&event->comm, sizeof(event->comm));
    bpf_probe_read_user_str(&event->filename, sizeof(event->filename), ag->filename);
...
int tracepoint_syscalls__sys_exit_open(struct trace_event_raw_sys_exit *ctx) {
...
static __always_inline int trace_exit(struct trace_event_raw_sys_exit *ctx)
{
	...
	event->uid = bpf_get_current_uid_gid();
	bpf_get_current_comm(&event->comm, sizeof(event->comm));
	bpf_probe_read_user_str(&event->filename, sizeof(event->filename), ap->fname);
