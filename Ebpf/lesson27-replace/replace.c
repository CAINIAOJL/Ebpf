...
const char *argp_program_version = "replace 1.0";
...
"     ./textrepalce -f /proc/modules -i ppdev -r $'aaaa\\n'"
"\n"
...
static const struct argp_option opts[] = {
    ...
    {"input", 'i', "INPUT", 0, "Text to replaced in file, max 20 chars"},
    {"replace", 'r', "REPLACE", 0, "Text to replace with in file, must be sme size as -t"},
    {"target-ppid", 't', "PPID", 0, "Optional Parent PID, will affect its children. "},
    {},
};
    ...
    switch (key) {
        case 'i':
            if(strlen(arg) >= text_len_max) {
                fprintf(stderr, "Input text too long, max %d chars\n", text_len_max);
                argp_usage(state);
            }
        ...
        case 'r' :
            if(strlen(arg) >= text_len_max) {
                fprintf(stderr, "Replace text too long, max %d chars\n", text_len_max);
                argp_usage(state);
            }
...
int main(int argc, char **argv) {
    ...
    if(env.filename[0] == '\x00' || env.input == '\x00' ||env.replace[0] == '\x00') {
        printf("Please provide all required arguments\n");
        exit(1);
    }

    if(strlen(env.input) != strlen(env.replace)) {
        printf("Input and replace text must be the same size\n");
        exit(1);
    }
